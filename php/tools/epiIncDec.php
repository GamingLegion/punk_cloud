...
$result = mysqli_query( $connect3, "SELECT season_rank, watched FROM $user WHERE epi_num = $i AND anime_name = '$rom_name' AND anime_season = '$season'" );
   ...
   if ( $incdec == 0 ) {
      ...
      if ( $watched > 1 ) {
         ...
         if ( mysqli_num_rows( $result ) < 1 ) {
            mysqli_query( $connect1, "UPDATE anime SET addedScore = addedScore - $rank, numOfRanks = numOfRanks - 1, addedWatch = addedWatch - 1 WHERE rom_name = '$rom_name' AND season = '$season'" );
         }
      }
   }
}
